...
module.exports = function (options) {
  ...
  var requestIdHeader = options.requestIdHeader || 'X-Request-Id'
  ...
  return function errorHandler (err, req, res, next) {
    ...
    !error.requestId && req.headers && req.headers[requestIdHeader] && (error.requestId = req.headers[requestIdHeader])
    if (env === 'development' && !error.stack) {
      tmpError = {}
      Error.captureStackTrace(tmpError, errorHandler)
